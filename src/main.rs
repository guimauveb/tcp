...
fn listen_remote(
    nic: &mut Iface,
    buf: &mut [u8],
    connections: &mut HashMap<Quad, TransmissionControlBlock>,
) -> io::Result<()> {
            ...
            Ok(tcp_header) => {
                ...
                match connections.entry(Quad {
                    ...
                    Entry::Occupied(mut entry) => {
                        ...
                        tcb.on_packet(nic, ip_header, tcp_header, &buf[data_offset..nbytes])?;
                    }
                    Entry::Vacant(entry) => {
                        if let Ok(Some(tcb)) = TransmissionControlBlock::accept(
                            nic,
                            ip_header,
                            tcp_header,
...
pub fn listen(
    ...
    match connections.entry(Quad { local, remote }) {
...
pub fn send(
    ...
    _remote: Socket,
    _data: &[u8],
    _connections: &mut HashMap<Quad, TransmissionControlBlock>,
) -> Result<(), Error> {
    unimplemented!();
}
...
fn main() -> io::Result<()> {
    let mut connections: HashMap<Quad, tcp::TransmissionControlBlock> = HashMap::default();
